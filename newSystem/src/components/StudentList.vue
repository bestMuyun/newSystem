...
export default {
    ...
    setup() {
        ...
        const searchList = () => {
            axios.post("http://10.17.226.10:8080/user/student/list", form).then(res => {
        ...
        const editUser = () => {
            axios.post("http://10.17.226.10:8080/user/edit", editForm).then(res => {
        ...
        const deleteStudent = (row) => {
            ElMessageBox.confirm(`此操作将永久删除<span style='color: red'>${row.name}</span>学生, 是否继续?`, "学生信息删除", {
                ...
                axios.delete("http://10.17.226.10:8080/user/student/delete/" + row.id).then(res => {
                    if (res.data.code === 200) {
                        ElMessage.success(res.data.msg);
                        searchList();
                    } else {
